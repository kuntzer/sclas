...
demo = sclas.SClas('demo_multipsf')
datadir = "../../data/multipsf_noisechange"
datadir_test = "../../data/600psf_noisechange"

datadir = "../../data/archive_data/spectra_10psf_HST"
datadir_test = "../../data/archive_data/spectra_600psf_HST"
Lambda = 0.5
...
outcat = sclas.utils.around(outcat, 0.5, amin=1, amax=7.5)
...
sclas.plots.scatter(ax0, comp[:,0], comp[:,1], c=truth_test, x_name=r"Component 1", 
	y_name=r"Component 2", c_name="Spectral type", 
	colorbarticks=np.arange(2, len(all_stellar_types))/2., colorbarticklabels=all_stellar_types)
if save_figures:
...
plt.plot([np.amin(truth_test),np.amax(truth_test)], [np.amin(truth_test),np.amax(truth_test)], 
		lw=2, color='red', ls='--')
sclas.plots.scatter(ax, outcat+uncertainty, truth_test, c=outcat.reshape(len(outcat)), x_name="Classification", 
